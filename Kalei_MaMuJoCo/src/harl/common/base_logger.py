...
class BaseLogger:
    ...
    def episode_log(
        ...
        if len(self.done_episodes_rewards) > 0:
            ...
            wandb.log(
                {
                    "train_episode_aver_rewards": aver_episode_rewards,
                    "timestep": self.total_num_steps,
                }
            )
            self.done_episodes_rewards = []
    ...
    def log_train(self, actor_train_infos, critic_train_info):
        ...
        for agent_id in range(self.num_agents):
            for k, v in actor_train_infos[agent_id].items():
                ...
                wandb.log({agent_k: v, "timestep": self.total_num_steps})
        # log critic
        for k, v in critic_train_info.items():
            critic_k = "critic/" + k
            wandb.log({critic_k: v, "timestep": self.total_num_steps})

    def log_env(self, env_infos):
        """Log environment information."""
        for k, v in env_infos.items():
            if len(v) > 0:
                wandb.log({k: np.mean(v), "timestep": self.total_num_steps})
